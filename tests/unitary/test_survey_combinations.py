...
from src.adapters.types import TotalBundle
from src.core.use_cases.survey_combinations import SurveyCombinations
from tests.mocks.raw import ALIAS_COMBINATION_PARAMS, KEYWORDS, RAW_HEADERS_OK
...
async def test_survey_two_keywords(mocker: Mocker):
    mocker.patch.dict(ALIAS_COMBINATION_PARAMS, {"keywords": KEYWORDS[:2]})
    ...
    for keyword in ALIAS_COMBINATION_PARAMS["keywords"]:
        assert keyword in combinations_repr


@mark.asyncio
async def test_survey_three_keywords(mocker: Mocker):
    mocker.patch.dict(ALIAS_COMBINATION_PARAMS, {"keywords": KEYWORDS[:3]})
    params = CombinationParams(**ALIAS_COMBINATION_PARAMS)
    results, _ = await USE_CASE.process(params)
