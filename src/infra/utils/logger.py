...
def batch_to_fetch(progress: range) -> None:
    LOGGER.info(
        "Batch of \033[33m%d\033[m requests to fetch\033[m",
        (progress.stop - progress.start),
        stacklevel=2,
    )
...
def batch_completed(process_time: float) -> None:
    ...
    LOGGER.info("Batch completed in \033[33m%s\033[m", duration, stacklevel=2)
...
def quota(
    ...
    LOGGER.log(
        _Level.QUOTA,
        "Scopus API: \033[32m%(api_name)s\033[m. Limit: \033[33m%(limit)s"
        "\033[m. Remaining: \033[33m%(remaining)s\033[m. Reset: \033[33m"
        "%(reset)s\033[m. ELS-Status: \033[33m%(status)s\033[m",
        args,
        stacklevel=2,
    )
...
def try_again(reset: str) -> None:
    LOGGER.info("Please try again on \033[37;1m%s\033[m", reset, stacklevel=2)


def localhost(port: int) -> None:
    LOGGER.info(
        "Please access at \033[37;1mhttp://localhost:%d\033[m",
        port,
        stacklevel=2,
    )
...
def total_found(total_results: int) -> None:
    LOGGER.info("Total Found: \033[33m%d\033[m", total_results, stacklevel=2)


def gunicorn_running(env_config: Json) -> None:
    LOGGER.info(
        "Active Configuration (Production-only Forced Overrides Applied): "
        "%s\033[m",
        to_jsonable_python(env_config, fallback=repr),
        stacklevel=2,
    )
    LOGGER.info(
...
def exception(exc: Exception) -> None:
    exc_trace = exc.__traceback__

    if exc_trace is not None:
        frame = traceback.extract_tb(exc_trace)[-1]
    else:
        frame = traceback.FrameSummary(
            filename="unknown", lineno=0, name="unknown", colno=0
        )
...
class _Trace:
    METHOD_COLOR = {"GET": "94", "POST": "92", "PUT": "93", "DELETE": "91"}
    STATUS_COLOR = {2: "32", 3: "33", 4: "31", 5: "31"}
    # e.g. apiKey=6bd9327547a3cf4c56586324df4b7d92  (Random Hash)
    API_KEY_PARAM_RE = re.compile(r"apiKey\=([a-zA-Z0-9]{32})")
    ...
    @classmethod
    def mask_api_key_param(cls, url: str) -> str:
        return cls.API_KEY_PARAM_RE.sub(cls._replacer, url)
    ...
    def log(
        ...
        args = {
            "host": host,
            "port": port,
            "method_color": cls.METHOD_COLOR.get(req.method, "90"),
            "method": req.method,
            "url": cls.mask_api_key_param(str(req.url)),
            "status_color": cls.STATUS_COLOR.get((code // 100), "30"),
            "code": code,
        ...
        LOGGER.log(
            prefix,
            "[\033[36m%(host)s\033[m:\033[36m%(port)d\033[m] \033["
            "%(method_color)sm%(method)s \033[37;1m%(url)s\033[m \033["
            "%(status_color)sm%(code)d %(status_phrase)s \033[m%(time)s\033[m",
            args,
            stacklevel=3,
        )
...
def trace(req: FastAPIRequest, code: int, process_time: float) -> None:
    if process_time > 60.0:
        ...
        duration = f"{process_time:.2f}s"
    _Trace.log(_Level.ACCESS, req, code, duration)
...
def api_call(url: str, code: int, time: float) -> None:
    ...
    _Trace.log(_Level.API_CALL, FastAPIRequest(scope), code, f"{time:.2f}s")
