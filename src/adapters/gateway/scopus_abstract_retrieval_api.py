...
from src.core.common.messages import INTERRUPT_ERROR
...
class ScopusAbstractRetrievalAPI(AbstractAPIABC):
    ...
    def __get_abstract(self, index: int) -> None:
        ...
        if index == range(self.__total).stop:
            self.__survey_detail.set_response(response)

        abstract: ScopusAbstract = self.__scopus_response.validate(response)
